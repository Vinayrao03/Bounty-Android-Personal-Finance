package com.example.vinay.personalfinance.database;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public  static final String COLUMN_TRANSACTION_ISEXPENSE ="trans_is_expense";
    ...
    public List<Account> getAccount(String id){
        ...
        if ((cursor.moveToFirst()) && cursor.getCount() !=0){
            ...
            account.setAccountName(cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_NAME)));
            account.setBalanceAmt(cursor.getFloat(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_BALANCE)));
            account.setId(cursor.getLong(cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_ID))));
            list.add(account);
            ...
            while (!cursor.isLast()) {
                ...
                account.setAccountName(cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_NAME)));
                account.setBalanceAmt(cursor.getFloat(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_BALANCE)));
                account.setId(cursor.getLong(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_ACCOUNT_ID)));
                list.add(account);
            }
